...
If you are installing this module into an ongoing campaign, your existing map assets will not automatically update until they are prompted. Here is how to handle the transition:

**1. Automating Token Vision & Detection**
To force the module to sweep your world and apply the correct vision shaders and detection modes to all pre-existing tokens:

- Go to **Game Settings** and click **Configure Settings**.
- Navigate to the **Module Settings** tab and find **RMU Lighting and Vision**.
- Toggle the **Enable Automated Lighting Engine** setting **OFF** and click **Save Changes**.
- Reopen the settings menu, toggle the engine back **ON**, and click **Save Changes** again.
- *Result:* The engine will read the character sheets of every token currently on the canvas, parsing their talents and silently injecting the correct native vision settings (such as Darkvision or Life Sense) and their ranges.
...
- **Magical Darkness & Utterdark:** Mechanical support for negative luminosity, allowing Utterdark to suppress ambient and magical light sources.
- **Active Effects Integration:** Support for spells and potions temporarily granting vision talents.
- **Expanded Game Settings:** UI menus allowing GMs to customise the exact ranges, colours, and exclusions for all detection and vision modes.
